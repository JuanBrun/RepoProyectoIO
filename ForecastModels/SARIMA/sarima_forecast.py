...
try:
    ...
    # Guardar gráfico
    output_path = os.path.join(script_dir, 'sarima_forecast.png')
    ...
    diag_path = os.path.join(script_dir, 'sarima_diagnostics.png')
    fig_diag.savefig(diag_path, dpi=150, bbox_inches='tight')
    print(f"✓ Diagnostico guardado: {diag_path}")
    plt.close(fig_diag)
    ...
    # Exportar a CSV
    results_path = os.path.join(script_dir, 'sarima_results.csv')
    results_df.to_csv(results_path, index=False)
    
    forecast_path = os.path.join(script_dir, 'sarima_forecast.csv')
    forecast_export_df.to_csv(forecast_path, index=False)
    
    print(f"✓ Resultados historicos: {results_path}")
    print(f"✓ Pronostico: {forecast_path}")

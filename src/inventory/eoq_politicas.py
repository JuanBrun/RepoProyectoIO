...
eoq_politicas.py
================
Modelo EOQ clásico con validación de CV (Winston pág. 872-873).
Incluye: Política A (costo óptimo) y Política B (nivel de servicio 95%).



Entrada: outputs/forecast/prophet_results.csv, prophet_forecast.csv
Salida:  outputs/inventory/politica_a_resultados.csv, politica_b_resultados.csv, etc.
"""
...
# 1. Demanda promedio: d_prom = (1/n) * sum(d_i)
d_promedio = np.mean(demandas_pronostico)

# 2. Varianza estimada: Var.Est.D = (1/n) * sum(d_i^2) - d_prom^2
var_est_d = np.mean(demandas_pronostico**2) - d_promedio**2

# 3. Coeficiente de Variabilidad: CV = Var.Est.D / d_prom^2
...
print(f"\n    d_prom (demanda promedio):    ${d_promedio:>15,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.'))
print(f"    Var.Est.D (varianza):          {var_est_d:>15,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.'))
print(f"    CV = Var.Est.D / d_prom^2:      {CV:>15.4f}")
...
print("\n" + "="*70)
print("POLÍTICA B: BASADA EN NIVEL DE SERVICIO (alpha = 0.05, Servicio = 95%)")
print("="*70)
...
print(f"\n  Z-score para alpha=0.05: {Z_ALPHA:.4f}")
...
print("\n--- Comparación detallada por componente ---")
print(f"\n{'Componente':<35} {'CTE Pol.A':<15} {'CTE Pol.B':<15} {'SS (Pol.B)':<12} {'Delta Costo':<12}")
print("-" * 89)
...
# Guardar gráfico
output_path = os.path.join(output_dir, 'eoq_comparacion.png')
...
print(f"[OK] Política A: {politica_a_path}")

# Guardar resultados de Política B
politica_b_path = os.path.join(output_dir, 'politica_b_resultados.csv')
df_politica_b.to_csv(politica_b_path, index=False)
print(f"[OK] Política B: {politica_b_path}")
...
comparacion_path = os.path.join(output_dir, 'comparacion_politicas.csv')
...
print("""
======================================================================
                                    POLITICAS DE INVENTARIO EOQ
----------------------------------------------------------------------
    POLITICA A (Optima por Costos)
        - CTE Total:              ${cte_total_a:>15,.2f}
        - Capacidad Almacen:      {volumen_total_a:>15,.2f} m3
        - Nivel de Servicio:      Variable (sin stock seguridad)
        - Riesgo:                 Mayor exposicion a agotamientos
----------------------------------------------------------------------
    POLITICA B (Nivel de Servicio 95%)
        - CTE Total:              ${cte_total_b:>15,.2f}
        - Capacidad Almacen:      {volumen_total_b:>15,.2f} m3
...
----------------------------------------------------------------------
    DIFERENCIA
        ...
        - Espacio adicional:      {volumen_total_b - volumen_total_a:>15,.2f} m3
======================================================================
